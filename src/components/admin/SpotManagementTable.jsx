...
const SpotManagementTable = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (!isAuthenticated || !token) throw new Error('User is not authenticated');
      await axios.delete(`/api/spots/${spotToDelete.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      // Update context or refetch spots as needed
  ...
  const handleSaveChanges = async () => {
    ...
    const updatedSpot = {
      ...
      location: {
        type: 'Point',
        coordinates: [
          parseFloat(document.getElementById('formSpotLocationLon').value) || 0,
          parseFloat(document.getElementById('formSpotLocationLat').value) || 0,
        ],
      },
  ...
  const handleAddNewSpot = async () => {
    try {
      ...
      formData.append('location[coordinates][0]', lon);
      ...
      formData.append('view360[imageUrl]', newSpot.view360.imageUrl || '');
      ...
      if (err.response?.status === 400) {
        alert('Failed to add spot: ' + (err.response.data.errors?.map(e => e.msg).join(', ') || 'Invalid input'));
        ...
        <Modal.Body>
          {spotToEdit && (
            <Form>
              ...
              <Form.Group className="mb-3" controlId="formSpotLocationLon">
                <Form.Label>Longitude</Form.Label>
                <Form.Control
                  type="number"
                  step="0.0001"
                  defaultValue={spotToEdit.location?.coordinates[0] || 0}
                />
              </Form.Group>
              <Form.Group className="mb-3" controlId="formSpotLocationLat">
                <Form.Label>Latitude</Form.Label>
                <Form.Control
                  type="number"
                  step="0.0001"
                  defaultValue={spotToEdit.location?.coordinates[1] || 0}
                />
              </Form.Group>
            ...
            <Form.Group className="mb-3" controlId="formNewSpotLocationLon">
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setNewSpot({
                    ...newSpot,
                    location: { ...newSpot.location, coordinates: [parseFloat(e.target.value) || 0, newSpot.location.coordinates[1]] },
                  })
                }
                required
              />
            ...
            <Form.Group className="mb-3" controlId="formNewSpotLocationLat">
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setNewSpot({
                    ...newSpot,
                    location: { ...newSpot.location, coordinates: [newSpot.location.coordinates[0], parseFloat(e.target.value) || 0] },
                  })
                }
                required
              />
